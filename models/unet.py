...
if __name__ == '__main__':
    model = UNet(3, 3).cuda()
    input = torch.zeros((1, 3, 400, 400)).cuda()
   # m = nn.AvgPool2d(2).cuda()
   # input2 = m(input).cuda()
   # n = nn.Conv2d(3,256,3, padding=1).cuda()
   # input3 = n(input2)
   # print(input3.shape)
    x7, out6, _, _, _, output9 = model(input)
    hello = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True)(x7)
    convolution = nn.Conv2d(128, 3,1).cuda()
    hello2 = convolution(hello).cuda()
    print(x7.shape)
    print(hello.shape)
    print(out6.shape)
  #  print(side7.shape)